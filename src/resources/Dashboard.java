...
import domain.Enrollment; // Added for reading enrollment status
    ...
    public Dashboard(User user) {
        ...
        if (currentUser instanceof AcademicOfficer || currentUser instanceof CourseAdministrator) {
            ...
            btnApproval.addActionListener(e -> {
                ...
                new resources.EnrollmentApprovalView(currentUser, this).setVisible(true);
            });
            ...
            btnManageUsers.addActionListener(e -> {
                ...
                new resources.StudentManagementView(currentUser).setVisible(true);
            });
            menuPanel.add(Box.createVerticalStrut(5));
        ...
        btnCheck.addActionListener(e -> {
            ...
            new resources.CheckRecoveryEligibility(currentUser).setVisible(true);
        });

        btnFailed.addActionListener(e -> {
            setActiveButton(btnFailed);
            new resources.FailedComponentOverview(currentUser).setVisible(true);
        });

        btnRec.addActionListener(e -> {
            setActiveButton(btnRec);
            new resources.RecommendationEntry(currentUser).setVisible(true);
        });

        btnSet.addActionListener(e -> {
            setActiveButton(btnSet);
            new resources.MilestoneActionPlan(currentUser).setVisible(true);
        });

        btnMonitor.addActionListener(e -> {
            setActiveButton(btnMonitor);
            new resources.RecoveryProgress(currentUser).setVisible(true);
        });

        btnAccount.addActionListener(e -> {
            setActiveButton(btnAccount);
            new resources.ManageAccountView(currentUser).setVisible(true);
        });
